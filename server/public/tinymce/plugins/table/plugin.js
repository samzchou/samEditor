...
(function () {
	...
	var extractAdvancedStyles = function (dom, elm) {
		...
		var getTdBorder = function () {
			return getTDTHOverallStyle(dom, elm, 'border-width') || getTDTHOverallStyle(dom, elm, 'border');
		};
		const tdBorderWidth = getTdBorder() || '1px';
		return {
	...
	const getSettingConfi = function (key){
	...
	var extractDataFromSettings = function (editor, hasAdvTableTab) {
		...
		const tableConfig = getSettingConfi('table');
		...
		var defaultData = {
			...
			class: '',
			align: '',
			border: tableConfig?.borderWidth || '2',
			tdBorder: tableConfig?.tdBorderWidth || '1',
		};
		...
		const borderWidth = getBorder();
		//console.log('borderWidth==>', borderWidth.border)
		if (borderWidth?.border) {
			defaultData.tdBorder = borderWidth.border;
		}
	...
	var extractDataFromTableElement = function (editor, elm, hasAdvTableTab) {
		...
		var getTdBorder = function (dom, elm) {
			return getTDTHOverallStyle(editor.dom, elm, 'border-width') || getTDTHOverallStyle(editor.dom, elm, 'border');
		};
		var dom = editor.dom;
		...
		const tdBorder = getTdBorder(dom, elm) || '1px';
		...
		return __assign({
			...
			title: !!dom.select('caption.table-title', elm)[0],
			description: !!dom.select('caption.description', elm)[0],
		...
		if (shouldStyleWithCss(editor) && tableElm.children) {
			for (var i = 0; i < tableElm.children.length; i++) {
				styleTDTH(dom, tableElm.children[i], {
					'border-width': addPxSuffix(data.tdBorder) || addPxSuffix(data.border),
	...
	var onSubmitTableForm = function (editor, tableElm, oldData, api) {
		...
		editor.undoManager.transact(function () {
			...
			if (size(modifiedData) > 0) {
				...
				// 表标题
				var titleElm = dom.select('caption.table-title', tableElm)[0];
				if (titleElm && !data.title) {
					dom.remove(titleElm);
				}
				if (!titleElm && data.title) {
					titleElm = dom.create('caption', { class:'table-title' }, tinymce.i18n.translate('table title'));
					tableElm.insertBefore(titleElm, tableElm.firstChild);
				}
				// 表的附加描述
				var descElm = dom.select('caption.description', tableElm)[0];
				if (descElm && !data.description) {
					dom.remove(descElm);
				}
				// debugger
				if (!descElm && data.description) {
					descElm = dom.create('caption', { class:'description' }, tinymce.i18n.translate('description'));
			...
			editor.execCommand('setTableAttrs', tableElm);
		});
	};
	var open = function (editor, insertNewTable) {
		...
		if(data.width && parseFloat(data.width) && parseFloat(data.width) > 100) {
			data.width = '100%';
		}
	...
	const isRevision = function() {
		let config = sessionStorage.getItem('tinymceConfig');
		if (config) {
		...
		var eraseTable = function () {
			return getSelectionStartCellOrCaption(editor).each(function (cellOrCaption) {
				table(cellOrCaption, isRoot).filter(not(isRoot)).each(function (table) {
					// 修订模式下的删除表格
					if (isRevision()) {
						editor.execCommand('removeTable', table.dom);
						return;
					}
					var cursor = SugarElement.fromText('');
					var tableContent = table.dom.outerHTML;
	...
	var addButtons = function (editor, selections, selectionTargets, clipboard) {
		editor.ui.registry.addMenuButton('table', {
			...
			fetch: function (callback) {
				return callback('inserttable | cell row column | advtablesort | tableprops autoTable deletetable');
			}
		});
		...
		editor.ui.registry.addButton('autoTable', {
			tooltip: '自动适应表高',
			onAction: cmd('autoTable'),
			onSetup: selectionTargets.onSetupTable
		});
	...
	var addMenuItems = function (editor, selections, selectionTargets, clipboard) {
		...
		editor.ui.registry.addMenuItem('autoTable', autoTable);
			...
			update: function () {
				...
				return selectionTargets.targets().fold(constant(''), function (targets) {
					if (name(targets.element) === 'caption') {
						return 'tableprops autoTable deletetable';
					} else {
						return 'cell row column | advtablesort | tableprops autoTable deletetable';
					}
				});
			}
		});
