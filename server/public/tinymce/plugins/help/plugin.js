...
(function () {
    ...
    var tab = function () {
	  ...
	  var tinymceConfig = sessionStorage.getItem('tinymceConfig');
	  tinymceConfig = JSON.parse(tinymceConfig);
	  var version = tinymceConfig?.editorVersion || '1.0.0';
	  
      var changeLogLink = `<a href="${tinymceConfig.mainUrl||''}" target="_blank">SamEditor ${version}</a>`;
      var htmlPanel = {
        type: 'htmlpanel',
    ...
    var parseCustomTabs = function (editor, customTabs) {
      ...
      var tabs = __assign((_a = {}, _a[shortcuts.name] = shortcuts, _a[nav.name] = nav, _a[versions.name] = versions, _a), customTabs.get());
