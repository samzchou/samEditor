...
(function () {
    ...
    var getSelectedCodeSample = function (editor, element) {
    ...
    var insertCodeSample = function (editor, language, code) {
      editor.undoManager.transact(function () {
        ...
        return node.fold(function () {
		  ...
		  editor.execCommand('putCode')
        }, function (n) {
          editor.dom.setAttrib(n, 'class', 'language-' + language);
		  n.innerHTML = code;
          get(editor).highlightElement(n);
          editor.selection.select(n);
		  editor.execCommand('putCode', n, code);
        });
      });
    };
    var getCurrentCode = function (editor, element) {
		var node = getSelectedCodeSample(editor, element);
		return node.fold(constant(''), function (n) {
			const reg = new RegExp(tinymce.i18n.translate('Copy Code'),'i')
			return n.textContent.replace(reg, '');
		});
    };
    ...
    function Plugin () {
      global$2.add('codesample', function (editor) {
		...
		editor.addCommand('copyCode', element => {
            const currCode = getCurrentCode(editor, element);
			editor.execCommand('copied', currCode);
        })
      });
    }
