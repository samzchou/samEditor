...
(function () {
	...
	var isSafeImageUrl = function (editor, src) {
		return true;// global$3.isDomSafe(src, 'img', editor.settings);
	};
	...
	var hasCaption = function (image) {
		...
		if (parentNode) {
			...
			if (nextNode && nextNode.classList.contains('img-title')) {
				return true;
			}
		}
		return false;
	...
	var getDocConfig = function () {
		let configs = sessionStorage.getItem('tinymceConfig');
		if (configs) {
	...
	var wrapInFigure = function (image) {
		...
		var figureElm = DOM.create('p', { class: 'image', 'data-id':uuid });
		DOM.insertAfter(figureElm, image);
		figureElm.appendChild(image);
		
		var titleContent = tinymce.i18n.translate('image title')
		var titleElm = DOM.create('p', { class: 'img-title', 'data-id':uuid }, titleContent);
		DOM.insertAfter(titleElm, figureElm);
		// 修订插入
		const configs = getDocConfig();
		if (configs?.revision) {
			
			const spanNode = DOM.create('span', {}, titleContent);
			spanNode.dataset.tag = 'revision';
			spanNode.dataset.revisionId = randomString();
			spanNode.dataset.author = configs?.author?.userName || 'test00001';
			spanNode.dataset.uid = configs?.author?.userId || 'test00001';
			spanNode.dataset.date = getLocalISOTime();
			spanNode.dataset.revisionType = 'insert';
			
			titleElm.innerHTML = '';
			titleElm.appendChild(spanNode);
		}
		/*
		var figureElm = DOM.create('figure', { class: 'image' });
		DOM.insertAfter(figureElm, image);
		figureElm.appendChild(image);
		figureElm.appendChild(DOM.create('figcaption', { class: 'img-title', contentEditable: 'true' }, tinymce.i18n.translate('image title')));
	...
	var removeFigure = function (image) {
		const configs = getDocConfig();
		// debugger
		var figureElm = image.parentNode.nextSibling;
		if (figureElm && figureElm.classList.contains('img-title')) {
			// 修订删除
			if (configs?.revision) {
				debugger
				const contents = figureElm.innerHTML;
				const spanNode = DOM.create('span', {}, '∇');
				spanNode.dataset.tag = 'revision';
				spanNode.dataset.revisionId = randomString();
				spanNode.dataset.author = configs?.author?.userName || 'test00001';
				spanNode.dataset.uid = configs?.author?.userId || 'test00001';
				spanNode.dataset.date = getLocalISOTime();
				spanNode.dataset.contents = contents;
				spanNode.dataset.revisionType = 'delete';
				
				figureElm.innerHTML = '';
				figureElm.appendChild(spanNode);
			} else {
	...
	var read = function (normalizeCss, image) {
		return {
			...
			alignCenter: getAttrib(image, 'align') || image.parentNode && getStyle(image.parentNode, 'textAlign')=='center',
	...
	var insertImageAtCaret = function (editor, data) {
		...
		if (isFigure(insertedElm)) {
			...
			// insertedElm.align = "left";
			editor.selection.select(insertedElm);
		}
		const parentNode = insertedElm.parentNode;
		// 居中
		// debugger
		if (data.alignCenter) {
			parentNode.style.textAlign = "center";
			//insertedElm.align = "center";
		} else if (data.imgAlign) {
			parentNode.style.textAlign = data.imgAlign;
			//insertedElm.align = data.imgAlign;
		}

		// 处理图标题
		editor.execCommand('toggleImageTitle', insertedElm, data);
	};
	...
	var getConfig = function (key) {
		let configs = sessionStorage.getItem('pageConfig');
		if (configs) {
	...
	var fromImageData = function (image) {
		const config = getConfig('imageTitle') || {};
		// const docConfig = getConfig('docSetting') || {};
		// 显示图标题
		image.caption = config?.showTitle;
		// 图标题居中
		image.alignCenter = config?.titleAlign === 'center';
