const nodeRSA = require("node-rsa");

const _pubKey = `-----BEGIN PUBLIC KEY-----
  MIGfMA0GCSqGSIb3DQEBAQUAA4GNADCBiQKBgQCK3siV2MKl7ADFMajEsbc/ZrST
  fo9h37c6+m0cUHOTqGR4V+Ugzc5Wzpcrz6QGu7/umQBJRC3LZ8xRW8+J6Z1lI0+T
  r6LT8NfLUeyBTBXkBI1j0BIzmEjsW/a1vDr2ahXn1RFvtnHeKs41lbICkY7mRA2c
  AyiMWhrteM1d1MR3gQIDAQAB
  -----END PUBLIC KEY-----`;

const _priKey = `-----BEGIN RSA PRIVATE KEY-----
  
  lEYH3FAsW+VP5dQ6AuzW+wkFDbztdvr5MF9Mluyxc3Fe
  -----END RSA PRIVATE KEY-----`;

module.exports = {
    // 加密
    encrypt(txt) {
        const encryptor = new nodeRSA(_pubKey, 'pkcs1-private', { encryptionScheme: "pkcs1" });
        encryptor.setOptions({ environment: "browser" });
        return encryptor.encrypt(txt, "utf8"); // 对数据进行加密
    },
    // 解密
    decrypt(txt) {
        try {
            const encryptor = new nodeRSA(_priKey, 'pkcs1-private', { encryptionScheme: "pkcs1" });
            encryptor.setOptions({ environment: "browser" });
            return encryptor.decrypt(txt, "utf8"); // 对数据进行解密
        } catch (error) {
            console.log(error);
            return "";
        }
    },
};
