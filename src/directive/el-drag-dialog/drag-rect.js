    ...
    bind(el, binding, vnode) {
        ...
        let moveDown = (e) => {
            ...
            document.onmousemove = (evt) => {
                // 通过事件委托，计算移动的距离
                const l = evt.clientX - disX;
                const t = evt.clientY - disY;
                // 移动当前元素
        ...
        resizeEl.style.zIndex = '99'; //鼠标拉伸弹窗
        resizeEl.onmousedown = (e) => {
            ...
            document.onmousemove = (evt) => {
                ...
                // 通过事件委托，计算移动的距离
                let x = evt.clientX - disX + (evt.clientX - clientX); //这里 由于elementUI的dialog控制居中的，所以水平拉伸效果是双倍
                let y = evt.clientY - disY;
                //比较是否小于最小宽高
        ...
        resizeElR.style.top = '0px'; //鼠标拉伸弹窗
        resizeElR.onmousedown = (e) => {
            ...
            document.onmousemove = (evt) => {
                ...
                if (clientX > EloffsetLeft + elW - 10 && clientX < EloffsetLeft + elW) {
                    //往左拖拽
                    if (clientX > evt.clientX) {
                        if (dragDom.clientWidth < minWidth) {} else {
                            dragDom.style.width = elW - (clientX - evt.clientX) * 2 + 'px';
                        }
                    }
                    //往右拖拽
                    if (clientX < evt.clientX) {
                        dragDom.style.width = elW + (evt.clientX - clientX) * 2 + 'px';
                    }
                }
            };
            //拉伸结束
        ...
        resizeElB.style.bottom = '0px'; //鼠标拉伸弹窗
        resizeElB.onmousedown = (e) => {
            ...
            document.onmousemove = (evt) => {
                e.preventDefault(); // 移动时禁用默认事件
                //底部鼠标拖拽位置
                if (ELscrollTop + clientY > EloffsetTop + elH - 20 && ELscrollTop + clientY < EloffsetTop +
                    elH) {
                    //往上拖拽
                    if (clientY > evt.clientY) {
                        if (dragDom.clientHeight < minHeight) {} else {
                            dragDom.style.height = elH - (clientY - evt.clientY) * 2 + 'px';
                        }
                    }
                    //往下拖拽
                    if (clientY < evt.clientY) {
                        dragDom.style.height = elH + (evt.clientY - clientY) * 2 + 'px';
                    }
                }
            };
            //拉伸结束
