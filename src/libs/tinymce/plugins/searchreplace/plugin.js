...
(function() {
    ...
    var typeOf = function(x) {
        ...
        if (x === null) {
            return 'null';
        } else if (t === 'object' && (Array.prototype.isPrototypeOf(x) || x.constructor && x.constructor
                .name === 'Array')) {
            return 'array';
        } else if (t === 'object' && (String.prototype.isPrototypeOf(x) || x.constructor && x.constructor
                .name === 'String')) {
            return 'string';
        } else {
            return t;
    ...
    var isContentEditableTrueInCef = function(dom, node) {
        return dom.getContentEditable(node) === 'true' && dom.getContentEditableParent(node.parentNode) ===
            'false';
    };
    ...
    var isBoundary = function(dom, node) {
        return isSimpleBoundary(dom, node) || isContentEditableFalse(dom, node) || isHidden(dom, node) ||
    ...
    var collectRangeSections = function(dom, rng) {
        ...
        return collect(dom, rng.commonAncestorContainer, startNode, endNode, {
            ...
            cef: function(node) {
                var sections = bind(descendants(SugarElement.fromDom(node),
                    '*[contenteditable=true]'), function(e) {
                    var ceTrueNode = e.dom;
                    return collect(dom, ceTrueNode, ceTrueNode);
                });
                return sort(sections, function(a, b) {
                    return documentPositionPreceding(a.elements[0].dom, b.elements[0].dom) ?
                        1 : -1;
                });
            }
        }, false);
    ...
    var findAndMark = function(dom, pattern, node, replacementNode, container) {
        var textSections = fromNode(dom, node);
        var matches = find$1(pattern, textSections, container);
    ...
    var markAllMatches = function(editor, currentSearchState, pattern, inSelection, container, id) {
        var marker = editor.dom.create('span', { 'data-mce-bogus': 1 });
        marker.className = 'mce-match-marker';
        if (id) {
            marker.dataset.id = id;
        }
        var node = container || editor.getBody();
        done(editor, currentSearchState, false);
        if (inSelection) {
            return findAndMarkInSelection(editor.dom, pattern, editor.selection, marker);
        } else {
            return findAndMark(editor.dom, pattern, node, marker, container, id);
        }
    };
    ...
    var escapeSearchText = function(text, wholeWord) {
        var escapedText = text.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, '\\$&').replace(/\s/g,
            '[^\\S\\r\\n\\uFEFF]');
        var wordRegex = '(' + escapedText + ')';
        return wholeWord ? '(?:^|\\s|' + punctuation() + ')' + wordRegex + ('(?=$|\\s|' + punctuation() + ')') :
            wordRegex;
    };
    var find = function(editor, currentSearchState, text, matchCase, wholeWord, inSelection, container, id) {
        var escapedText = escapeSearchText(text, wholeWord);
        var pattern = {
            regex: new RegExp(escapedText, matchCase ? 'g' : 'gi'),
            matchIndex: 1
        };
        var count = markAllMatches(editor, currentSearchState, pattern, inSelection, container, id);
        if (count) {
            if (!container) {
    ...
    var open = function(editor, currentSearchState) {
        ...
        var scale = pageContainer.style.transform || '';
        scale = scale.replace(/scale|\(|\)/g, '');
        if (scale && parseFloat(scale) > 1) {
            editor.windowManager.alert('使用查找和替换功能前请先将页面缩小至100%');
            return;
        }
        ...
        var doFind = function(api) {
            ...
            if (last.text === data.findtext && last.matchCase === data.matchcase && last.wholeWord === data
                .wholewords) {
                next(editor, currentSearchState);
            } else {
    ...
    var register$1 = function(editor, currentSearchState) {
        ...
        editor.addCommand('highlightText', function(obj) {
            done(editor, currentSearchState, false);
            if (obj.text) {
                return find(editor, currentSearchState, obj.text, false, false, false, obj.container, obj.id);
            } else {
                return done(editor, currentSearchState);
            }
        });
        editor.addCommand('clearHighlightt', function(obj) {
            done(editor, currentSearchState, false);
        });
    };
    ...
    var register = function(editor, currentSearchState) {
        ...
        setTimeout(() => {
            if (editor.settings.doc_config && !editor.settings.doc_config.reader) {
