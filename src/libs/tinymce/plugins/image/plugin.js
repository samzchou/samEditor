...
(function() {
    ...
    var getHspace = function(image) {
        if (image.style.marginLeft && image.style.marginRight && image.style.marginLeft === image.style
            .marginRight) {
    ...
    var getVspace = function(image) {
        if (image.style.marginTop && image.style.marginBottom && image.style.marginTop === image.style
            .marginBottom) {
    ...
    var setAlt = function(image, alt, isDecorative) {
        ...
        if (isDecorative) {
            ...
            sugarImage = SugarElement.fromDom(image);
            set(sugarImage, 'alt', '');
        } else {
            if (isNull(alt)) {
                sugarImage = SugarElement.fromDom(image);
    ...
    var makeItems = function(info) {
        ...
        return flatten([
            ...
            [__assign(__assign({}, getDialogContainerType(info.classList.isSome() && info
                .hasImageCaption)), {
                items: flatten([
                    classList.toArray(),
    ...
    var calcVSpace = function(css) {
        var matchingTopBottom = css['margin-top'] && css['margin-bottom'] && css['margin-top'] === css[
            'margin-bottom'];
        return matchingTopBottom ? removePixelSuffix(String(css['margin-top'])) : '';
    };
    var calcHSpace = function(css) {
        var matchingLeftRight = css['margin-right'] && css['margin-left'] && css['margin-right'] === css[
            'margin-left'];
    ...
    var changeHandler = function(helpers, info, state) {
        return function(api, evt) {
			var data = api.getData();
			/* debugger
			console.log(api.getData(), evt) */
			api.enable('save');
            if (evt.name === 'src') {
				if(data.src.meta.invalid) {
					api.disable('save');
					return false;
				}
                changeSrc(helpers, info, state, api);
        ...
        if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
            var tabPanel = {
                ...
                tabs: flatten([
                    ...
                    info.hasUploadTab && (info.hasUploadUrl || info.hasUploadHandler) ? [UploadTab
                        .makeTab(info)
                    ] : []
                ])
            };
    ...
    var submitHandler = function(editor) {
        return function(info) {
            return function(api) {
                var data = deepMerge(fromImageData(info.image), api.getData());
				if (!data.src.value) {
					editor.windowManager.alert("请选择图片上传！")
					return false;
				}
