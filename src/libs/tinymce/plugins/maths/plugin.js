tinymce.PluginManager.add('math', function(editor, url) {
    ...
    var filterItems = function() {
        var docConfig = editor.settings.doc_config;
        if(docConfig.mathItems && Array.isArray(docConfig.mathItems)) {
            return itemList.filter(item=> {
                return docConfig.mathItems.includes(item.cata);
            })
        } else {
            return itemList;
        }
    ...
    // 下拉工具栏菜单
    editor.ui.registry.addMenuButton('math', {
        icon: `math-btn`,
        text: 'math',
        tooltip: pluginName,
        fetch: callback => {
    ...
    // 菜单栏
    editor.ui.registry.addNestedMenuItem('math', {
        icon: 'math-btn',
        text: 'math',
