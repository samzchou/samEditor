...
(function() {
    ...
    var checkRange = function(str, substr, start) {
        return substr === '' || str.length >= substr.length && str.substr(start, start + substr.length) ===
            substr;
    };
    ...
    var browsers = [{
            ...
            search: function(uastring) {
                return contains$1(uastring, 'edge/') && contains$1(uastring, 'chrome') && contains$1(
                    uastring, 'safari') && contains$1(uastring, 'applewebkit');
            }
        },
        {
            name: 'Chrome',
        ...
        },
        {
            ...
            search: function(uastring) {
                return (contains$1(uastring, 'safari') || contains$1(uastring, 'mobile/')) && contains$1(
                    uastring, 'applewebkit');
            }
        }
    ];
    var oses = [{
    ...
    var bypassSelector = function(dom) {
        return dom.nodeType !== ELEMENT && dom.nodeType !== DOCUMENT && dom.nodeType !== DOCUMENT_FRAGMENT ||
    ...
    var one = function(selector, scope) {
        var base = scope === undefined ? document : scope.dom;
        return bypassSelector(base) ? Optional.none() : Optional.from(base.querySelector(selector)).map(
    ...
    var internalSet = function(dom, property, value) {
        if (!isString(value)) {
            console.error('Invalid call to CSS.set. Property ', property, ':: Value ', value, ':: Element ',
                dom);
        ...
        each(list, function(rowData) {
            if (rowData.section === 'colgroup') {
                ...
                each(rowData.cells, function(rowCell) {
                    ...
                    var current = extended(rowCell.element, rowCell.rowspan, rowCell.colspan,
                        rowCount, start, isLocked);
                    for (var occupiedColumnPosition = 0; occupiedColumnPosition < rowCell
                        .colspan; occupiedColumnPosition++) {
                        for (var occupiedRowPosition = 0; occupiedRowPosition < rowCell
                            .rowspan; occupiedRowPosition++) {
    ...
    var inSelection = function(bounds, detail) {
        ...
        return leftEdge <= bounds.finishCol && rightEdge >= bounds.startCol && (topEdge <= bounds.finishRow &&
    ...
    var getBounds = function(detailA, detailB) {
        return bounds(Math.min(detailA.row, detailB.row), Math.min(detailA.column, detailB.column), Math.max(
            detailA.row + detailA.rowspan - 1, detailB.row + detailB.rowspan - 1), Math.max(detailA
            .column + detailA.colspan - 1, detailB.column + detailB.colspan - 1));
    };
            ...
            return lookupTable(start).bind(function(startTable) {
                return lookupTable(finish).bind(function(finishTable) {
                    if (eq$1(startTable, finishTable)) {
                        ...
                        var finishCell = ancestorCells.length > 0 ? ancestorCells[ancestorCells
                            .length - 1] : finish;
                        return Optional.some({
                            boxes: nestedIntercepts(startTable, start, startTable,
                                finish, finishTable),
                            start: start,
                            finish: finishCell
                        });
                        ...
                        var startCell = ancestorCells.length > 0 ? ancestorCells[ancestorCells
                            .length - 1] : start;
                        return Optional.some({
                            boxes: nestedIntercepts(finishTable, start, startTable,
                                finish, finishTable),
                            start: start,
                            finish: startCell
                        });
                        ...
                        return ancestors$4(start, finish).shared.bind(function(lca) {
                            return closest$1(lca, 'table', isRoot).bind(function(
                                lcaTable) {
                                var finishAncestorCells = ancestors$1(finish,
                                    'td,th', getIsRoot(lcaTable));
                                var finishCell = finishAncestorCells.length >
                                    0 ? finishAncestorCells[finishAncestorCells
                                        .length - 1] : finish;
                                var startAncestorCells = ancestors$1(start,
                                    'td,th', getIsRoot(lcaTable));
                                var startCell = startAncestorCells.length > 0 ?
                                    startAncestorCells[startAncestorCells
                                        .length - 1] : start;
                                return Optional.some({
                                    boxes: nestedIntercepts(lcaTable,
                                        start, startTable, finish,
                                        finishTable),
                                    start: startCell,
                                    finish: finishCell
                                });
        ...
        return getEdges(container, firstSelectedSelector, lastSelectedSelector).bind(function(edges) {
            ...
            return firstAncestor.bind(function(fA) {
                return lastAncestor.bind(function(lA) {
                    return eq$1(fA, lA) ? getBox$1(edges.table, edges.first, edges
                        .last) : Optional.none();
                });
            });
        });
        ...
        each(cases, function(acase, count) {
            ...
            adt[key] = function() {
                ...
                if (argLength !== value.length) {
                    throw new Error('Wrong number of arguments to case ' + key + '. Expected ' +
                        value.length + ' (' + value + '), got ' + argLength);
                }
                    ...
                    fold: function() {
                        ...
                        if (foldArgs.length !== cases.length) {
                            throw new Error('Wrong number of arguments to fold. Expected ' +
                                cases.length + ', got ' + foldArgs.length);
                        }
    ...
    var absolute = function(element) {
        ...
        var scrollTop = firstDefinedOrZero(win === null || win === void 0 ? void 0 : win.pageYOffset, html
            .scrollTop);
        var scrollLeft = firstDefinedOrZero(win === null || win === void 0 ? void 0 : win.pageXOffset, html
            .scrollLeft);
    ...
    var getPercentSize = function(elm, getter) {
        ...
        return 100; // sam.shen modify2021-08-23 getter(elm) / getter(relativeParent) * 100;
    };
    ...
    var getWidthFrom = function(warehouse, table, getWidth, fallback, tableSize) {
        ...
        var backups = [Optional.some(width.edge(table))].concat(map(width.positions(columnCells, table),
            function(pos) {
    ...
    var getHeightFrom = function(warehouse, table, direction, getHeight, fallback) {
        ...
        var backups = [Optional.some(direction.edge(table))].concat(map(direction.positions(rows, table),
            function(pos) {
                return pos.map(function(p) {
                    return p.y;
                });
    ...
    var unmergable = function(selections) {
        ...
        return candidates.length > 0 && forall(candidates, hasRowOrColSpan) ? Optional.some(candidates) :
            Optional.none();
    };
        ...
        editor.on('BeforeGetContent', function(e) {
            var multiCellContext = function(cells) {
                ...
                extractSelected(cells).each(function(elements) {
                    e.content = e.format === 'text' ? getTextContent(elements) :
    ...
    var determine = function(input, column, step, tableSize, resize) {
        ...
        var onLeft = function(index, next) {
            return resize.calcLeftEdgeDeltas(result, index, next, step, tableSize.minCellWidth(), tableSize
                .isRelative);
        };
        var onMiddle = function(prev, index, next) {
            return resize.calcMiddleDeltas(result, prev, index, next, step, tableSize.minCellWidth(),
                tableSize.isRelative);
        };
        var onRight = function(prev, index) {
            return resize.calcRightEdgeDeltas(result, prev, index, step, tableSize.minCellWidth(), tableSize
                .isRelative);
        };
    ...
    var adjustAndRedistributeWidths = function(_table, list, details, tableSize, resizeBehaviour) {
        ...
        var _a = resizeBehaviour.calcRedestributedWidths(sizes, tablePixelWidth, details.pixelDelta, tableSize
                .isRelative),
    ...
    var reqMessage = function(required, keys) {
        throw new Error('All required keys (' + sort$1(required).join(', ') +
    ...
    var invalidTypeMessage = function(incorrect, type) {
        throw new Error('All values need to be of type: ' + type + '. Keys (' + sort$1(incorrect).join(', ') +
            ') were not.');
    };
    var checkDupes = function(everything) {
        ...
        dupe.each(function(d) {
            throw new Error('The field: ' + d + ' occurs more than once in the combined fields: [' +
                sorted.join(', ') + '].');
        });
    };
    ...
    var refreshCol = function(wire, colPositions, position, tableHeight) {
        drawBar(wire, colPositions, function(origin, cp) {
            var colBar = col(cp.col, cp.x - origin.left, position.top - origin.top, BAR_THICKNESS,
                tableHeight);
    ...
    var refreshRow = function(wire, rowPositions, position, tableWidth) {
        drawBar(wire, rowPositions, function(origin, cp) {
            var rowBar = row(cp.row, position.left - origin.left, cp.y - origin.top, tableWidth,
                BAR_THICKNESS);
            ...
            mutation.get().each(function(target) {
                hoverTable.each(function(table) {
                    getResizer(target, 'data-row').each(function(row) {
                        ...
                        events.trigger.adjustHeight(table, delta, parseInt(row,
                            10));
                    });
                    getResizer(target, 'data-column').each(function(column) {
                        var delta = getDelta(target, 'left');
                        remove(target, 'data-initial-left');
                        events.trigger.adjustWidth(table, delta, parseInt(
                            column, 10));
                    });
                    refresh(wire, table);
                });
    ...
    // sam modify 2021-08-06 新增表格对齐(tableAlignLeft tableAlignCenter tableAlignRight)
    var defaultTableToolbar =
        'tableprops tabledelete | tableinsertrowbefore tableinsertrowafter tabledeleterow | tableinsertcolbefore tableinsertcolafter tabledeletecol | tableAlignLeft tableAlignCenter tableAlignRight';
    var defaultStyles = {
    ...
    var defaultAttributes = { border: '2' };
    ...
    var get$a = function(editor, isResizable) {
        return editor.inline ? ResizeWire.body(getBody$1(editor), createContainer(), isResizable) : ResizeWire
            .only(SugarElement.fromDom(editor.getDoc()), isResizable);
    };
    ...
    var getResizeHandler = function(editor) {
        ...
        var afterCornerResize = function(table, origin, width) {
            ...
            if (width !== startW && startRawW !== '') {
                ...
                var tableSize = lazySizing(table);
                var col = isPreserveTableColumnResizing(editor) || isRightEdgeResize ? getNumColumns(
                    table) - 1 : 0;
        ...
        editor.on('init', function() {
            ...
            if (hasObjectResizing(editor) && hasTableResizeBars(editor)) {
                ...
                sz.events.beforeResize.bind(function(event) {
                    ...
                    fireObjectResizeStart(editor, rawTable, getPixelWidth$1(rawTable),
                        getPixelHeight(rawTable), barResizerPrefix + event.type);
                });
                sz.events.afterResize.bind(function(event) {
                    ...
                    fireObjectResized(editor, rawTable, getPixelWidth$1(rawTable),
                        getPixelHeight(rawTable), barResizerPrefix + event.type);
        ...
        editor.on('ObjectResizeStart', function(e) {
            ...
            if (isTable(targetElm)) {
                ...
                each(editor.dom.select('.mce-clonedresizable'), function(clone) {
                    editor.dom.addClass(clone, 'mce-' + getColumnResizingBehaviour(editor) +
                        '-columns');
                });
    ...
    var freefallRtl = function(universe, element) {
        ...
        return children.length > 0 ? freefallRtl(universe, children[children.length - 1]) : point(candidate,
        ...
        return map(grid, function(row, rowIndex) {
            var details = bind(row.cells, function(cell, columnIndex) {
                if (seen[rowIndex][columnIndex] === false) {
                    ...
                    return [detailnew(cell.element, result.rowspan, result.colspan, cell
                    .isNew)];
                } else {
                    return [];
                }
    ...
    var measure = function(startAddress, gridA, gridB) {
        if (startAddress.row >= gridA.length || startAddress.column > cellLength(gridA[0])) {
            return Result.error('invalid start address out of table bounds, row: ' + startAddress.row +
                ', column: ' + startAddress.column);
        }
    ...
    var tailor = function(gridA, delta, generators) {
        ...
        var modifiedCols = fillCols(gridA, Math.abs(delta.colDelta), generators, isLastColLocked ? gridWidth -
            1 : gridWidth);
        var newLockedColumns = getLockedColumnsFromGrid(modifiedCols);
        return fillRows(modifiedCols, Math.abs(delta.rowDelta), generators, mapToObject(newLockedColumns,
            always));
    };
    ...
    var isSpanning = function(grid, row, col, comparator) {
        ...
        return grid.length > 1 && cellLength(currentRow) > 1 && (col > 0 && matching(getCellElement(currentRow,
                col - 1)) || col < currentRow.cells.length - 1 && matching(getCellElement(currentRow, col +
                1)) || row > 0 && matching(getCellElement(grid[row - 1], col)) || row < grid.length - 1 &&
    ...
    var merge$1 = function(startAddress, gridA, gridB, generator, comparator) {
        ...
        var lockedColumnsWithinBounds = getLockedColumnsWithinBounds(validStartAddress, gridBRows,
            lockedColumns);
        var result = measure(validStartAddress, gridA, gridBRows);
        return result.map(function(diff) {
            var delta = __assign(__assign({}, diff), { colDelta: diff.colDelta -
                    lockedColumnsWithinBounds.length });
            var fittedGrid = tailor(gridA, delta, generator);
            var newLockedColumns = getLockedColumnsFromGrid(fittedGrid);
            var newLockedColumnsWithinBounds = getLockedColumnsWithinBounds(validStartAddress,
                gridBRows, newLockedColumns);
    ...
    var insertRows = function(index, gridA, gridB, generator, comparator) {
        ...
        var secondDelta = __assign(__assign({}, secondDiff), { colDelta: secondDiff.colDelta + newLocked
                .length });
        var fittedGridB = lockedColFill(gridB, generator, newLocked);
        var fittedNewGrid = tailor(fittedGridB, secondDelta, generator);
        return oldCols.concat(oldRows.slice(0, index)).concat(fittedNewGrid).concat(oldRows.slice(index, oldRows
            .length));
    };
    ...
    var insertRowAt = function(grid, index, example, comparator, substitution) {
        ...
        var between = mapCells(rows[example], function(ex, c) {
            var withinSpan = index > 0 && index < rows.length && comparator(getCellElement(rows[index -
                1], c), getCellElement(rows[index], c));
            var ret = withinSpan ? getCell(rows[index], c) : elementnew(substitution(ex.element,
                comparator), true, ex.isLocked);
            return ret;
        });
    ...
    var insertColumnAt = function(grid, index, example, comparator, substitution) {
        return map(grid, function(row) {
            var withinSpan = index > 0 && index < cellLength(row) && comparator(getCellElement(row,
                index - 1), getCellElement(row, index));
            var sub = getElementFor(row, index, row.section, withinSpan, example, comparator,
                substitution);
    ...
    var replaceColumn = function(grid, index, comparator, substitution) {
        ...
        var targets = bind(rows, function(row, i) {
            var alreadyAdded = notStartRow(grid, i, index, comparator) || notStartColumn(row, index,
                comparator);
    ...
    var replaceRow = function(grid, index, comparator, substitution) {
        ...
        var targets = bind(targetRow.cells, function(item, i) {
            var alreadyAdded = notStartRow(rows, index, i, comparator) || notStartColumn(targetRow, i,
                comparator);
        ...
        var markCell = function(cell) {
            return last$1(cell).bind(function(rightEdge) {
                ...
                return parent(rightEdge).map(function(parent) {
                    return rightSiblingIsBlock === true || isListItem(parent) || isBr(
                        rightEdge) || isBlock$1(parent) && !eq$1(cell, parent) ? [] : [
    ...
    var opInsertRowsBefore = function(grid, details, comparator, genWrappers) {
        ...
        var newGrid = foldr(rows, function(acc, row) {
            var newG = insertRowAt(acc.grid, targetIndex, row.row + acc.delta, comparator, genWrappers
                .getOrInit);
            return {
                grid: newG,
                delta: acc.delta + 1
    ...
    var opInsertColumnsBefore = function(grid, extractDetail, comparator, genWrappers) {
        ...
        var newGrid = foldr(columns, function(acc, col) {
            var newG = insertColumnAt(acc.grid, targetIndex, col.column + acc.delta, comparator,
    ...
    var opPasteColsAfter = function(grid, pasteDetails, comparator, _genWrappers) {
        var rows = extractGridDetails(grid).rows;
        var index = pasteDetails.cells[pasteDetails.cells.length - 1].column + pasteDetails.cells[pasteDetails
            .cells.length - 1].colspan;
        var context = rows[pasteDetails.cells[0].row];
        var gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
        var mergedGrid = insertCols(index, grid, gridB, pasteDetails.generators, comparator);
        var cursor = elementFromGrid(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    ...
    var opPasteRowsAfter = function(grid, pasteDetails, comparator, _genWrappers) {
        var rows = extractGridDetails(grid).rows;
        var index = pasteDetails.cells[pasteDetails.cells.length - 1].row + pasteDetails.cells[pasteDetails
            .cells.length - 1].rowspan;
        var context = rows[pasteDetails.cells[0].row];
        var gridB = gridifyRows(pasteDetails.clipboard, pasteDetails.generators, context);
        var mergedGrid = insertRows(index, grid, gridB, pasteDetails.generators, comparator);
        var cursor = elementFromGrid(mergedGrid, pasteDetails.cells[0].row, pasteDetails.cells[0].column);
    ...
    var insertColumnsBefore = run(opInsertColumnsBefore, insertColumnsExtractor(true),
        adjustAndRedistributeWidths$1, noop, Generators.modification);
    var insertColumnsAfter = run(opInsertColumnsAfter, insertColumnsExtractor(false), adjustAndRedistributeWidths$1,
        noop, Generators.modification);
    var eraseColumns = run(opEraseColumns, eraseColumnsExtractor, adjustAndRedistributeWidths$1, prune, Generators
        .modification);
    ...
    var makeColumnsHeader = run(opMakeColumnsHeader, onUnlockedCells, noop, noop, Generators.transform('row',
    'th'));
    var unmakeColumnHeader = run(opUnmakeColumnHeader, onUnlockedCell, noop, noop, Generators.transform(null,
    'td'));
    var unmakeColumnsHeader = run(opUnmakeColumnsHeader, onUnlockedCells, noop, noop, Generators.transform(null,
        'td'));
    ...
    var switchSectionType = function(editor, rowElm, newType) {
        ...
        if (newType === 'header') {
            ...
            var headerRowType = headerRowTypeSetting === 'auto' ? determineHeaderRowType() :
    ...
    var getSelectionStartFromSelector = function(selector) {
        return function(start, isRoot) {
            ...
            var startCell = startCellName === 'col' || startCellName === 'colgroup' ?
                getSelectionStartCellFallback(start) : start;
    ...
    var TableActions = function(editor, lazyWire, selections) {
        ...
        var execute = function(operation, guard, mutate, lazyWire, effect) {
            return function(table, target) {
                ...
                var resizeBehaviour = isResizeTableColumnResizing(editor) ? resizeTable() :
                    preserveTable();
                return guard(table) ? operation(wire, table, target, generators, sizing,
                    resizeBehaviour).bind(function(result) {
        ...
        var insertColumnsBefore$1 = execute(insertColumnsBefore, always, colMutationOp, lazyWire,
            structureModified);
        var insertColumnsAfter$1 = execute(insertColumnsAfter, always, colMutationOp, lazyWire,
            structureModified);
        ...
        var setTableCellType = function(editor, args) {
            return extractType(args, ['td', 'th']).each(function(type) {
                var cells = map(getCellsFromSelection(getSelectionStart(editor), selections),
                    function(c) {
                        return c.dom;
                    });
        ...
        var setTableRowType = function(editor, args) {
            return extractType(args, [
                ...
                map(getRowsFromSelection(getSelectionStart(editor), ephemera.selected), function(
                    row) {
    ...
    var createRow = function(columns, rowHeaders, columnHeaders, rowIndex, rows) {
        ...
        for (var j = 0; j < columns; j++) {
            ...
            // 最后一格单元格换为占位符，防止光标移动后跳转到其他地方
            if(j == columns-1 && (rowIndex + 1) == rows){
                td.dom.innerHTML = '&#8203';
            } else {
                append(td, SugarElement.fromTag('br'));
            }
            append(tr, td);
    ...
    var createRows = function(rows, columns, rowHeaders, columnHeaders) {
        return range(rows, function(r) {
            return createRow(columns, rowHeaders, columnHeaders, r, rows);
        });
    };
    var render$1 = function(rows, columns, rowHeaders, columnHeaders, headerType, renderOpts, editor) {
        ...
        // modify sam.shen 非航司的模板需要表头
		if (!editor.settings.doc_config || !editor.settings.doc_config?.tempPath || editor.settings.doc_config?.tempPath !== 'hsTmp') {
			var captionTitle = SugarElement.fromTag('caption');
			captionTitle.dom.className = 'table-title';
			captionTitle.dom.dataset.number = "1";
			captionTitle.dom.textContent = '表标题';
			append(table, captionTitle);
		}
    ...
    var insert$1 = function(editor, columns, rows, colHeaders, rowHeaders) {
		...
		if (!defaultStyles.border) {
			defaultStyles.border = '2px solid #333;';
		}
        var options = {
        ...
        editor.undoManager.ignore(function() {
            var table = render$1(rows, columns, rowHeaders, colHeaders, getTableHeaderType(editor), options, editor);
    ...
    var copyRows = function(table, target, generators) {
        ...
        return details.bind(function(selectedCells) {
            ...
            var rows = extractGridDetails(grid).rows;
            var slicedGrid = rows.slice(selectedCells[0].row, selectedCells[selectedCells.length - 1]
                .row + selectedCells[selectedCells.length - 1].rowspan);
            var filteredGrid = bind(slicedGrid, function(row) {
                var newCells = filter(row.cells, function(cell) {
                    return !cell.isLocked;
                });
                return newCells.length > 0 ? [__assign(__assign({},
                row), { cells: newCells })] : [];
            });
    ...
    var extractDataFromSettings = function(editor, hasAdvTableTab) {
        ...
        var data = __assign(__assign(__assign(__assign(__assign(__assign({}, defaultData), style), attrs),
            advStyle), getBorder()), getCellPaddingCellSpacing());
        return data;
    };
    var extractDataFromTableElement = function(editor, elm, hasAdvTableTab) {
        ...
        return __assign({
            ...
            caption: !!dom.select('caption.table-title', elm)[0],
            description: !!dom.select('caption.table-description', elm)[0],
        ...
        if (size(modifiedData) > 0 && cells.length >= 1) {
            ...
            getSelectedCells(cells).each(function(selectedCells) {
                each(selectedCells, function(item) {
                    ...
                    var modifier = isSingleCell ? DomModifier.normal(editor, cellElm) :
                        DomModifier.ifTruthy(editor, cellElm);
                    var colModifier = item.column.map(function(col) {
                        return isSingleCell ? DomModifier.normal(editor, col) :
    ...
    var getData = function(editor, cells) {
        var cellsData = getSelectedCells(cells).map(function(selectedCells) {
            return map(selectedCells, function(item) {
                return extractDataFromCellElement(editor, item.element, hasAdvancedCellTab(
                    editor), item.column);
            });
        });
    ...
    var applyRowData = function(editor, rows, oldData, data) {
        ...
        if (size(modifiedData) > 0) {
            each(rows, function(rowElm) {
                ...
                var modifier = isSingleRow ? DomModifier.normal(editor, rowElm) : DomModifier.ifTruthy(
                    editor, rowElm);
    ...
    var onSubmitRowForm = function(editor, rows, oldData, api) {
        var data = api.getData();
        // 判断是否为CLONE表
        var table = editor.dom.getParent(rows[0],'table');
        if (table && table.dataset.parentid && data.type === 'header') {
            editor.windowManager.alert('续表不可设置表头！');
            return false;
        }
        api.close();
        editor.undoManager.transact(function() {
    ...
    var getItems$2 = function(editor, classes, insertNewTable) {
        ...
        return rowColCountItems.concat(alwaysItems).concat(alignmentItem).concat(appearanceItems).concat(classListItem);
    };
    ...
    var onSubmitTableForm = function(editor, tableElm, oldData, api) {
        ...
        editor.undoManager.transact(function() {
            ...
            if (size(modifiedData) > 0) {
                ...
                // 表标题
                titleElm = dom.select('caption.table-title', tableElm)[0];
                if (titleElm && !data.caption) {
                    dom.remove(titleElm);
                }
                if (!titleElm && data.caption) {
                    titleElm = dom.create('caption', {class:'table-title'});
                    titleElm.innerHTML = !global$2.ie ? '<br data-mce-bogus="1"/>' : nbsp;
                    tableElm.insertBefore(titleElm, tableElm.firstChild);
                }
                // 表的附加描述
                descElm = dom.select('caption.table-description', tableElm)[0];
                if (descElm && !data.description) {
                    dom.remove(descElm);
                }
                if (!descElm && data.description) {
                    descElm = dom.create('caption', {class:'table-description'});
                    descElm.innerHTML = !global$2.ie ? '此处输入附加信息' : nbsp;
                    tableElm.insertBefore(descElm, tableElm.querySelector('tbody'));
                }
    ...
    var registerCommands = function(editor, actions, cellSelection, selections, clipboard) {
        ...
        var setSizingMode = function(sizing) {
            return getSelectionStartCellOrCaption$1(editor).each(function(cellOrCaption) {
                var isForcedSizing = isResponsiveForced(editor) || isPixelsForced(editor) ||
        ...
        var actOnSelection = function(execute) {
            return getSelectionStartCell$1(editor).each(function(cell) {
                getTableFromCell(cell).each(function(table) {
                    // addBy: sam.shen 2021-09-22
                    var td = cell.dom;
                    var tr = td.parentNode;
                    tinyMCE.tableCell = {
                        'cellIndex': editor.dom.nodeIndex(td),
                        'rowIndex': editor.dom.nodeIndex(tr),
                        'cell': cell.dom,
                        'isThead': tr.parentNode.nodeName === 'THEAD',
                        'table': table.dom
                    }
        ...
        var copyRowSelection = function() {
            return getSelectionStartCell$1(editor).map(function(cell) {
                return getTableFromCell(cell).bind(function(table) {
                    var targets = forMenu(selections, table, cell);
                    var generators = cellOperations(noop, SugarElement.fromDom(editor
                        .getDoc()), Optional.none());
            ...
            return getRows().each(function(rows) {
                ...
                getSelectionStartCell$1(editor).each(function(cell) {
                    return getTableFromCell(cell).each(function(table) {
                        var generators = paste(SugarElement.fromDom(editor
                        .getDoc()));
                        var targets = pasteRows(selections, cell, clonedRows,
                            generators);
                        execute(table, targets).each(postExecute(table));
                    });
                });
        ...
        editor.addCommand('mceInsertTable', function(_ui, args) {
            ...
            if (editor.dom.hasClass(block, 'fixed') || editor.dom.hasClass(block, 'preface')) {
                editor.windowManager.alert('当前页面不可插入表格！');
                return false;
            } */
        ...
        // sam 新增事件(表格对齐)
        editor.addCommand('mceTableAlignLeft', function(_ui, args) {
            editor.formatter.apply('table_align', { value: 'left' });
        });

        editor.addCommand('mceTableAlignCenter', function(_ui, args) {
            editor.formatter.apply('table_align', { value: 'center' });
        });
        editor.addCommand('mceTableAlignRight', function(_ui, args) {
            editor.formatter.apply('table_align', { value: 'right' });
        });
    };
    ...
    var next = function(current, isRoot) {
        ...
        return detection.fold(function() {
            ...
            return info.index + 1 < info.all.length ? CellLocation.middle(current, info.all[info.index +
                1]) : CellLocation.last(current);
        });
    };
    var prev = function(current, isRoot) {
        ...
        return detection.fold(function() {
            ...
            return info.index - 1 >= 0 ? CellLocation.middle(current, info.all[info.index - 1]) :
    ...
    var fromRange = function(win, type, range) {
        return type(SugarElement.fromDom(range.startContainer), range.startOffset, SugarElement.fromDom(range
        ...
        return selection.match({
            ...
            exact: function(start, soffset, finish, foffset) {
                return {
                    ...
                    rtl: cached(function() {
                        return Optional.some(exactToNative(win, finish, foffset, start,
                            soffset));
                    })
                };
            }
    ...
    var doDiagnose = function(win, ranges) {
        ...
        if (rng.collapsed) {
            ...
            return reversed.map(function(rev) {
                return adt$5.rtl(SugarElement.fromDom(rev.endContainer), rev.endOffset, SugarElement
                    .fromDom(rev.startContainer), rev.startOffset);
            }).getOrThunk(function() {
    ...
    var searchFromPoint = function(doc, x, y) {
        return SugarElement.fromPoint(doc, x, y).bind(function(elem) {
            ...
            return children(elem).length === 0 ? fallback() : searchTextNodes(doc, elem, x, y).orThunk(
                fallback);
        });
    };
            ...
            rtl: function(start, soffset, finish, foffset) {
                getNativeSelection(win).each(function(selection) {
                    if (selection.setBaseAndExtent) {
                        ...
                        try {
                            setLegacyRtlRange(win, selection, start, soffset, finish,
                                foffset);
                        } catch (e) {
    ...
    var readRange = function(selection) {
        if (selection.rangeCount > 0) {
            ...
            return Optional.some(SimRange.create(SugarElement.fromDom(firstRng.startContainer), firstRng
    ...
    var doGetExact = function(selection) {
        if (selection.anchorNode === null || selection.focusNode === null) {
            ...
            return after$3(anchor, selection.anchorOffset, focus_1, selection.focusOffset) ? Optional.some(
    ...
    var convertToRange = function(win, selection) {
        var rng = asLtrRange(win, selection);
        return SimRange.create(SugarElement.fromDom(rng.startContainer), rng.startOffset, SugarElement.fromDom(
        ...
        if (!eq$1(start, finish)) {
            return identify(start, finish, isRoot).bind(function(cellSel) {
                ...
                if (boxes.length > 0) {
                    ...
                    return Optional.some(Response.create(Optional.some(makeSitus(start, 0, start,
                        getEnd(start))), true));
                } else {
                    return Optional.none();
                }
    ...
    var update = function(rows, columns, container, selected, annotations) {
        ...
        return shiftSelection(selected, rows, columns, annotations.firstSelectedSelector, annotations
            .lastSelectedSelector).map(updateSelection);
    };
    ...
    var hone = function(universe, item, predicate, mode, direction, isRoot) {
        ...
        return next.bind(function(n) {
            if (isRoot(n.item)) {
                ...
                return predicate(n.item) ? Optional.some(n.item) : hone(universe, n.item, predicate, n
                    .mode, direction, isRoot);
            }
        });
    };
            ...
            return closest$1(before, 'td,th', isRoot).map(function(beforeCell) {
                if (!eq$1(afterCell, beforeCell)) {
                    return sharedOne$1(isRow, [
                        ...
                        return isOverlapping(bridge, beforeCell, afterCell) ? adt$6
                            .success() : failure(beforeCell);
                    }, function(_sharedRow) {
                        return failure(beforeCell);
                    });
                } else {
                    return eq$1(after, afterCell) && getEnd(afterCell) === afterOffset ?
                        failure(beforeCell) : adt$6.none('in same cell');
                }
            });
    ...
    var gatherer = function(cand, gather, isRoot) {
        return gather(cand, isRoot).bind(function(target) {
            return isText(target) && get$4(target).trim().length === 0 ? gatherer(target, gather,
                isRoot) : Optional.some(target);
        });
    };
    ...
    var tryBr = function(isRoot, element, offset, direction) {
        var target = isBr(element) ? handleBr(isRoot, element, direction) : handleParent(isRoot, element,
            offset, direction);
    ...
    var adjustDown = function(bridge, element, guessBox, original, caret) {
        ...
        if (Math.abs(guessBox.bottom - original.bottom) < 1) {
            ...
            return inOutsideBlock(bridge, element, caret) ? adt$7.retry(translate(lowerCaret, JUMP_SIZE, 0)) :
                adt$7.none();
        }
    };
    var adjustUp = function(bridge, element, guessBox, original, caret) {
        ...
        if (Math.abs(guessBox.top - original.top) < 1) {
            ...
            return inOutsideBlock(bridge, element, caret) ? adt$7.retry(translate(higherCaret, JUMP_SIZE, 0)) :
                adt$7.none();
        }
    };
    var upMovement = {
                ...
                return getEntireBox(bridge, element).bind(function(guessBox) {
                    return movement.adjuster(bridge, element, guessBox, original, caret)
                        .fold(Optional.none, function(newCaret) {
                            return adjustTil(bridge, movement, original, newCaret,
                                numRetries - 1);
                        });
                }).orThunk(function() {
                    return Optional.some(caret);
    ...
    var findSpot = function(bridge, isRoot, direction) {
        return bridge.getSelection().bind(function(sel) {
            return tryBr(isRoot, sel.finish, sel.foffset, direction).fold(function() {
                ...
                var analysis = BeforeAfter.verify(bridge, sel.finish, sel.foffset, range.finish,
                    range.foffset, direction.failure, isRoot);
    ...
    var scan$1 = function(bridge, isRoot, element, offset, direction, numRetries) {
        ...
        return tryCursor(bridge, isRoot, element, offset, direction).bind(function(situs) {
            ...
            var analysis = BeforeAfter.verify(bridge, element, offset, range.finish, range.foffset,
            ...
            return BeforeAfter.cata(analysis, function() {
                ...
                if (eq$1(element, cell) && offset === getEnd(cell)) {
                    ...
                    return scan$1(bridge, isRoot, cell, getEnd(cell), direction, numRetries -
                    1);
                }
            });
        });
    ...
    var handle$2 = function(bridge, isRoot, direction) {
        return findSpot(bridge, isRoot, direction).bind(function(spot) {
            return scan$1(bridge, isRoot, spot.element, spot.offset, direction, MAX_RETRIES).map(bridge
                .fromSitus);
        });
    };
        ...
        if (detect$3().browser.isIE()) {
            ...
            return precheck(initial, isRoot).orThunk(function() {
                return simulate(bridge, isRoot, direction, initial, anchor).map(function(info) {
                    ...
                    return Response.create(Optional.some(makeSitus(range.start, range.soffset,
                        range.finish, range.foffset)), true);
                });
            });
        }
    };
            ...
            return closest$1(startRow, 'table', isRoot).bind(function(table) {
                ...
                if (eq$1(startRow, rows[0])) {
                    return seekLeft$1(table, function(element) {
                        ...
                        return Response.create(Optional.some(makeSitus(last, lastOffset,
            ...
            return closest$1(startRow, 'table', isRoot).bind(function(table) {
                ...
                if (eq$1(startRow, rows[rows.length - 1])) {
                    return seekRight$1(table, function(element) {
                        ...
                        return Response.create(Optional.some(makeSitus(first, 0, first,
                            0)), true);
                    });
                } else {
                    return Optional.none();
    ...
    var MouseSelection = function(bridge, container, isRoot, annotations) {
        ...
        var mouseup = function(event) {
            ...
            clearstate();
            // sam add 2023-8-5
            tinyMCE.activeEditor.execCommand('tableSelection', boxes);
        };
        // debugger
        return {
        ...
        var keydown = function(event, start, soffset, finish, foffset, direction) {
            ...
            var handler = retrieve(container, annotations.selectedSelector).fold(function() {
                if (isDown(keycode) && shiftKey) {
                    return curry(select, bridge, container, isRoot, down, finish, start, annotations
                        .selectRange);
                } else if (isUp(keycode) && shiftKey) {
                    return curry(select, bridge, container, isRoot, up, finish, start, annotations
                        .selectRange);
                ...
                var update$1 = function(attempts) {
                    return function() {
                        var navigation = findMap(attempts, function(delta) {
                            return update(delta.rows, delta.cols, container,
                                selected, annotations);
                        });
                        return navigation.fold(function() {
                            return getEdges(container, annotations
                                .firstSelectedSelector, annotations
                                .lastSelectedSelector).map(function(edges) {
                                var relative = isDown(keycode) || direction
                                    .isForward(keycode) ? Situ.after : Situ
                                    .before;
                                bridge.setRelativeSelection(Situ.on(edges
                                    .first, 0), relative(edges
                                    .table));
                                annotations.clear(container);
                                return Response.create(Optional.none(),
                                    true);
                            });
                        }, function(_) {
                            return Optional.some(Response.create(Optional.none(),
                                true));
                        });
                    };
                };
        ...
        var keyup = function(event, start, soffset, finish, foffset) {
            return retrieve(container, annotations.selectedSelector).fold(function() {
                ...
                if (isNavigation(keycode)) {
                    return sync(container, isRoot, start, soffset, finish, foffset, annotations
                        .selectRange);
                } else {
                    return Optional.none();
                }
    ...
    var getDownOrRightCells = function(grid, selectedCells, generators) {
        ...
        return bind(downDetails, function(detail) {
            var slicedCells = detail.cells.slice(selectedCells[0].column + selectedCells[0].colspan - 1,
                detail.cells.length);
            return map(slicedCells, function(cell) {
                return cell.element;
            });
    ...
    function CellSelection(editor, lazyResize, selectionTargets) {
        var onSelection = function(cells, start, finish) {
            selectionTargets.targets().each(function(targets) {
                ...
                tableOpt.each(function(table) {
                    var cloneFormats = getCloneElements(editor);
                    var generators = cellOperations(noop, SugarElement.fromDom(editor.getDoc()),
                        cloneFormats);
        ...
        editor.on('init', function(_e) {
            ...
            var keydown = function(event) {
                ...
                var start = SugarElement.fromDom(rng.startContainer);
                var end = SugarElement.fromDom(rng.endContainer);
                var direction = onDirection(ltr$2, rtl$2)(SugarElement.fromDom(editor.selection
                    .getStart()));
    ...
    var getSelectionTargets = function(editor, selections) {
        ...
        var findTargets = function() {
            return getSelectionStartCellOrCaption(getSelectionStart(editor), getIsRoot(editor)).bind(
    ...
    var addButtons = function(editor, selectionTargets, clipboard) {
        ...
        var cmd = function(command) {
            return function() {
                var currNode = editor.selection.getNode();
                var table = editor.dom.getParent(currNode, 'table');
                if (editor.dom.hasClass(table.parentNode,'text-box')) {
                    currNode = editor.dom.getParent(currNode, 'td') || editor.dom.getParent(currNode, 'th');
                    let align = 'center';
                    switch(command) {
                    ...
                    currNode.align = align;
                    return true;
                }
                return editor.execCommand(command, table.dataset.id);
            };
        };
