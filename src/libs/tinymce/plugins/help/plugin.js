...
(function() {
    ...
    var doAction = function(editor, type) {
        var docConfig = editor.settings.doc_config;
        var url = docConfig.nodeURL + '/' + type + '/index.html';
        var title = "帮助说明";
        switch (type) {
        case 'tinymce/help':
                if (docConfig.useHelp && docConfig.useHelp.manual) {
                    editor.windowManager.alert("帮助说明：" + docConfig.useHelp.manual);
                    return;
                }
                break;
            case 'tinymce/concatus':
                title = "联系我们";
                if (docConfig.useHelp && docConfig.useHelp.concatus) {
                    editor.windowManager.alert("联系我们：" + docConfig.useHelp.concatus);
                    return;
                }
                break;
            case 'tinymce/version':
                title = "编辑器版本";
                break;
            case 'tinymce/upgrade': // 用于离线编辑器
                title = "在线升级";
				url += '?version=' + editor.settings.version
                break;
        }
        if (type === 'tinymce/version') {
            editor.windowManager.alert("编辑器当前版本：" + editor.settings.version);
            return;
        }
        // 弹窗以iframe显示内容
    ...
    var register$1 = function(editor, dialogOpener) {
        ...
        editor.ui.registry.addMenuItem('help', {
            ...
            // onAction: dialogOpener
            onAction: () => {
                doAction(editor, 'tinymce/help');
            }
        });
        editor.ui.registry.addMenuItem('bzt-ver', {
            text: '编辑器版本',
            icon: 'reload',
            onAction: () => {
                doAction(editor, 'tinymce/version');
            }
        });

        editor.ui.registry.addMenuItem('concatus', {
            text: '联系我们',
            onAction: () => {
                doAction(editor, 'tinymce/concatus');
            }
        });

        editor.ui.registry.addMenuItem('upgrade', {
            text: '在线升级',
            onAction: () => {
                doAction(editor, 'tinymce/upgrade');
            }
        });
    };
    ...
    var tab$2 = function(editor) {
        var availablePlugins = function() {
            ...
            return '<div data-mce-tabstop="1" tabindex="-1">' + '<p><b>' + global$2.translate(
                    'Premium plugins:') + '</b></p>' + '<ul>' + premiumPluginList +
                ...
                global$2.translate('Learn more...') + '</a></li>' + '</ul>' + '</div>';
        };
    ...
    var tab$3 = function() {
        ...
        var changeLogLink = '标准通编辑器 Ver.1.0.1';
        var htmlPanel = {
            type: 'htmlpanel',
    ...
    var parseCustomTabs = function(editor, customTabs) {
        ...
        var versions = tab$3();
        var tabs = __assign((_a = {}, _a[shortcuts.name] = shortcuts, _a[nav.name] = nav, _a[versions.name] =
            versions, _a), customTabs.get());
