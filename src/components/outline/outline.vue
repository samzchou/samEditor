<template>
    <div class="out-line-container">
        ...
        <el-tree ref="outlineTree" :data="treeData" :props="defaultProps" :empty-text="emptyText" :allow-drop="allowDrop" :allow-drag="allowDrag" @node-drop="nodeDropped" :draggable="draggable"
            @node-drag-over="handleDragOver" default-expand-all :highlight-current="!draggable" node-key="outlineId" :expand-on-click-node="false" @node-click="handleNodeClick">
            ...
            <div class="custom-tree-node" slot-scope="{ node, data }" :class="{'locked':!editOutline && isOtherOwner(data) && author.lockedAll }" :title="isOtherOwner(data)">
                ...
                <div :style="`flex:1; max-width:${node.level > 5 ? 200-(node.level-5)*30 : 200}px`" :data-owner="data.owner">
                    <span v-if="[8,9].includes(data.outlineType) && stdKind !== 901" class="bk" :class="data.docattr">
                    ...
                    <span v-else-if="stdKind === 901 && data.extendContent==='附录'" class="bk">附录{{ data.outlineCatalog }}</span>

                    <span v-else class="bk">{{ data.outlineCatalog && (![8,9].includes(data.outlineType) || stdKind === 901)  ? data.outlineCatalog : '-' }}</span>
                ...
                <template v-if="node.level>1 && author && (author.commitId || author.lockedAll)">
                    ...
                    <div v-else style="padding-right: 5px;">
                        ...
                        <el-button-group v-if="adminOutline && editOutline && !data.locked">
                            <el-button v-if="[5,6,8,9].includes(data.outlineType) && node.level===2" size="mini" icon="el-icon-document-copy" title="插入同类章节" @click.stop.prevent="() => insertChapter(data)" />
                            <!-- 分配大纲按钮 -->
                            <!-- <span>{{checkAssignOutlieAuthor(node, data)}}</span> -->
                            <el-button size="mini" class="ass1" :class="{'active': hasAssignOutline(data)}" :icon="!assignTitle(data.owner) ? 'el-icon-user' : 'el-icon-user-solid'" :title="assignTitle(data.owner)||'分配大纲'" @click.stop.prevent="() => assignOutline(node, data)" />
                            <!-- 删除节点 -->
                            <el-button v-if="(!isPrivate || ![1,3,4,5].includes(data.outlineType)) && !fixed.includes(data.outlineType) && node.level===2" size="mini" icon="el-icon-delete" title="删除节点" @click.stop.prevent="() => removeItem(node, data)" />
                        </el-button-group>
                        ...
                        <el-button-group v-else-if="author.lockedAll && !author.assignOutline">
                            ...
                            <el-button v-if="node.level===2 && !lockedByUser(data)" size="mini" :class="{'active':lockedBySelf(data)}" :icon="lockedBySelf(data) ? 'el-icon-unlock' : 'el-icon-edit-outline'" :title="lockedBySelf(data) ? '解锁章节' : '编辑章节'" @click.stop.prevent="editDocbyUnlock(data)" />
                        </el-button-group>
                        ...
                        <el-button-group v-else>
                            <!-- 可分配章节 -->
                            <template>
                                <el-button v-if="chechHasAssigned(data) && !checkHasLocked(data) && !data.locked" size="mini" class="ass3" :class="{'active':hasAssignOutline(data)}"
                                    :icon="!assignTitle(data.owner) ? 'el-icon-user' : 'el-icon-user-solid'" :title="assignTitle(data.owner)||'分配大纲'"
                            ...
                            <el-button v-if="!data.locked && checkIsAssignOutline(data)" size="mini" :class="{'active':lockedBySelf(data) && checkIsAssignOutline(data)}"
                                :icon="lockedBySelf(data) ? 'el-icon-unlock' : 'el-icon-edit-outline'" :title="lockedBySelf(data) ?'解锁-章节' : '编辑-章节'" @click.stop.prevent="editDocbyUnlock(data)" />
                        </el-button-group>
                ...
                <template v-else>
                    ...
                    <div v-if="data.parentId==='0' && checkSingleEdit && !merged && !author.commitId">
                        <el-menu v-if="stdKind===901" mode="horizontal" class="tree-menu" @select="handleSelectAdd">
                            <el-submenu index="0">
                                <template slot="title">新增</template>
                                <template v-for="(item,idx) in hsMenus">
                                    <el-menu-item :index="item.index" :key="idx">{{item.label}}</el-menu-item>
                                </template>
                            </el-submenu>
                        </el-menu>

                        <el-menu v-else mode="horizontal" class="tree-menu" @select="handleSelectAdd">
                            <el-submenu index="0">
                                <template slot="title">新增</template>
                    ...
                    <div v-else-if="node.level===2 && checkSingleEdit && !merged" class="btns">
                        ...
                        <template v-else>
                            <el-button v-if="[5,6,8,9].includes(data.outlineType)" size="mini" icon="el-icon-document-copy" title="插入同类章节" @click.stop.prevent="() => insertChapter(data)" />
                            <el-button v-if="(!isPrivate || ![1,3,4,5].includes(data.outlineType)) && !fixed.includes(data.outlineType)" size="mini" icon="el-icon-delete" title="删除节点"
        ...
        <!-- 弹窗组件-->
        <el-dialog append-to-body custom-class="cmp-dialog" :data-cmp="cmpName" :visible.sync="dialogData.visible" :width="dialogData.width" :fullscreen="dialogData.fullscreen"
            :close-on-click-modal="false" :show-close="false" destroy-on-close @before-close="closeDialog" @close="closeDialog">
...
<script>
    ...
    import {
        ...
        outlineMenus,
        hsMenus,
    } from "../tinymceEditor/configs/editorOptions";
    ...
    export default {
        ...
        watch: {
            data: {
                handler(data) {
                    if (!_.isEmpty(data) && !$global.compare(data, this.treeData)) {
                        setTimeout(() => {
                            this.setData(data);
                        }, 300);
                    }
        ...
        data() {
            return {
                ...
                assignOutlineId: [],
                scrollTop: 0,
                clickCount: 0, // 记录复制点击次数
                lastClickTime: null, // 上次点击时间
            };
        },
        methods: {
            // 以下这两函数处理拖拽位置的显示
            ...
            handleDragOver() {
                const dropIndicator = this.$refs.outlineTree.$el.querySelector(
            ...
            checkHasLocked(data) {
                ...
                // 校验父级点是否已经被锁定
                var parentNodes = $global.getParentNode(
                    this.treeData[0]["children"],
                    data.outlineId, { idKey: "outlineId", parentKey: "parentId", children: "children" }
                );
            ...
            chechHasAssigned(data) {
                ...
                if (this.author.assignOutline) {
                    if (this.author.isAdmin) {
                        return this.editOutline; // true;
                    }
                    // 已定义的可分配的章节子集,不包含本身已被指定的章节
                    if (!_.isEmpty(this.author.memberList)) {
                        for (let i = 0; i < this.assignOutlineId.length; i++) {
                            if (
                                data.ancestors.split(",").includes(this.assignOutlineId[i]) &&
                                !this.assignOutlineId.includes(data.outlineId)
                            ) {
                                flag = true;
                                break;
                            }
            ...
            checkIsAssignOutline(data) {
                if (!_.isEmpty(data.owner)) {
                    var owner = _.isString(data.owner) ?
            ...
            notChildAssign(node = {}, data = {}) {
                var lockedItem = _.find(this.lockedOutline, {
                    lockedOutlineId: data.outlineId,
                });
            ...
            allowDrag(node = {}) {
                console.log(
                    "this.editOutline===>",
                    this.editOutline,
                    this.author.lockedAll
                );
                // 协同模式下并锁定
                if (!this.editOutline && this.author.lockedAll) {
                    return false;
                }

                // 不允许拖拽的类型，1 前言 3 范围 4 规范性引用文件 5 术语和定义
                if (this.isStandard) {
                    const notAllowType = [1, 2, 11, 12]; // 3, 4, 5
            ...
            allowDrop(draggingNode = {}, currNode = {}, type = "") {
                ...
                if (type === "inner" && draggingNode.level === 2 && currNode.level >= 2) {
                    return false;
                } else if (draggingNode.level === 2 && currNode.level > 2) {
                    return false;
                }
                // 其他章节类型一致的
            ...
            nodeDropped(droppedNode, targetNode, type) {
                // 章节不能拖拽到内部中
                const resultTreeNode = this.$refs["outlineTree"].getNode(
                    droppedNode.data.parentId
                );
                // const targetNodeTreeNode = this.$refs['outlineTree'].getNode(targetNode.data.parentId);
                this.updateChildren(
                    this.treeData[0].children,
                    droppedNode.data.parentId,
                    resultTreeNode.data.children
                );
            ...
            assignTitle(owners) {
                if (!_.isEmpty(owners) && this.author && this.author.memberList) {
                    owners =
                        _.isString(owners) && $global.isJSON(owners) ?
                        JSON.parse(owners) :
                        owners;
                    const userName = [];
                    if (_.isArray(owners)) {
            ...
            lockedByUser(data = {}) {
                var lockedItem = _.find(this.lockedOutline, (obj) => {
                    return (
                        obj.lockedOutlineId === data.outlineId &&
                        obj.userId !== this.author.userId
                    );
                });
            ...
            lockedBySelf(data = {}, node = {}) {
                var lockedIndex = _.findIndex(this.lockedOutline, {
                    lockedOutlineId: data.outlineId,
            ...
            setData(data) {
                ...
                if (data && data.length) {
                        ...
                        if (arr && Array.isArray(arr)) {
                            arr.forEach((item) => {
                                if ([8, 9].includes(item.outlineType)) {
                                    item.docattr =
                                        item.infoNum == 8 || !item.infoNum ? "specs" : "means"; //item.outlineType == 8 ? 'specs' : 'means';
                                }
                                // item.aliasTitle = this.parseOutlineTitle(item.outlineTitle);
                                // 解锁
                                if (this.author.userId && this.author.lockedAll) {
                                    let lockIndex = _.findIndex(this.lockedOutline, (o) => {
                                        return (
                                            o.lockedOutlineId === item.outlineId && o.userId !== userId
                                        );
                                    });
                                ...
                                if (!_.isEmpty(item.owner) && !_.isEmpty(this.memberList)) {
                                    if (
                                        ...
                                        $global.isJSON(item.owner)
                                    ) {
                                        item.owner = JSON.parse(item.owner);
                                    }
                                    // console.log('owner==>', item.owner, typeof item.owner);
                                    this.memberList.forEach((user) => {
                                        let assignOutline = user.assignOutline ?
                                            user.assignOutline.split(",") :
                                            [];
                                        let assignIndex = _.findIndex(item.owner, {
                                            userId: user.userId,
                                        });
                                        if (!!~assignIndex) {
                                ...
                                // 层级锁定不能被分配
                                if (
                                    this.author.outlineLevel &&
                                    level > this.author.outlineLevel
                                ) {
                                    item.disabled = true;
                                }

                                if (
                                    parseInt(item.isVisible) === 1 ||
                                    (this.author.lockedAll && this.author.assignOutline)
                                ) {
                                    lists.push(item);
                                    // 继续遍历
                    ...
                    /*lists = lists.filter(item => {
                                    return item.outlineTitle !== '';
                                });*/
                    // 加上根节点
                    lists.unshift(treeData);
                    this.treeData = $global.handleTree(
                        lists,
                        "outlineId",
                        "parentId",
                        "children",
            ...
            updateItem(data = null) {
                /*const sourceItem = $global.findTreeByLeafId(data.outlineId, this.treeData[0].children, {key:"outlineId"});
                          if (sourceItem && sourceItem.outlineTitle !== data.outlineTitle) {
            ...
            resetClickWarning: _.debounce(function() {
            ...
            setLimit() {
                ...
                if (this.clickCount > 2) {
                    this.$notify.info({
                        title: "提示",
                        message: "您当前点击频繁，请稍后重试",
                    });
                    this.resetClickWarning();
                    return false;
                } else {
                    return true;
                }
            ...
            insertChapter(item = {}) {
                if (this.setLimit()) {
                    ...
                    let itemCatalog = String(item.outlineCatalog).split(".");
                    ...
                    let sameNodes = _.filter(treeData.children, {
                    ...
                    if (item.letter || [8, 9].includes(item.outlineType)) {
                        sameNodes = _.filter(treeData.children, (c) => {
                            return (
                                c.parentId === item.parentId && [8, 9].includes(c.outlineType) &&
                                c.isVisible === 1
                            );
                        });
                    }
                    // debugger
                    let index = _.findIndex(treeData.children, {
                        outlineId: item.outlineId,
                    });
                    let data = _.merge({}, item, {
                    ...
                    treeData.children.forEach((child) => {
                        ...
                        if (childItem) {
                            if (childItem.letter) {
                                childItem.letter = $global.numberToLetters(appendixIndex);
                                appendixIndex++;
                            }
                        }
                    });
                    ...
                    this.resetTreeData();
                    // 更新文档结构
                    // this.debouncedRefreshDoc(outlineId, data)
            ...
            handleSelectAdd(key, keyPath, vm = null, title = undefined) {
                ...
                var orderNum = this.getLastOrderNum(
                    treeData,
                    outlineType,
                    keyPath.includes("8-9")
                );
                // debugger
                if (!orderNum) {
                ...
                if (key === '8' && this.stdKind === 901) { // 航司附录章节
                    outlineType = '6'
                }
                ...
                switch (key) {
                    ...
                    case "12":
                        if (
                            _.find(treeData.children, (item) => {
                                return item.outlineType == data.outlineType;
                            })
                        ) {
                            error = "已存在相同类型大纲节点！";
                        }
                    ...
                    case "9":
                        ...
                        if (keyPath.includes("8-9")) {
                            ...
                            data.letter = $global.numberToLetters(outlineCatalog);
                            data.docattr = key === "8" ? "specs" : "means"; // 规范性 | 资料性 (outlineType)
                            // 同时需要添加子节点
                            data.children = [];
                        } else if (key === '8' && this.stdKind === 901) {
                            let appendixItems = _.filter(this.treeData[0]["children"], { outlineTitle:'附录' })
                            data.outlineTitle = "附录";
                            data.appendixNo = $global.setLowerToString(appendixItems.length);
                        }
                        break;
                        // 其他
                    default:
            ...
            refreshDoc(outlineId = "", data = null) {
                setTimeout(() => {
                    ...
                    let itemIndex = _.findIndex(children, { outlineId });
                    // debugger
                    let prevNode = children[itemIndex - 1];
                    if (this.stdKind === 901) {
                        if (data.appendixNo) {
                            prevNode = _.last(children);
                        } else {
                            children = children.filter(o=>o.outlineTitle !== '附录');
                            itemIndex = _.findIndex(children, { outlineId });
                            prevNode = children[itemIndex - 1];
                        }
                    }
                    // 上报新增章节页面事件
            ...
            getLastCatalog(isAppendix = false) {
                ...
                var lists = _.filter(this.treeData[0]["children"], (item) => {
                    return (
                        item["outlineCatalog"] !== undefined && [3, 4, 5, 6, 8, 9].includes(item.outlineType) && (isAppendix ? [8, 9].includes(item.outlineType) : ![8, 9].includes(item.outlineType))
                    );
                });
                if (this.stdKind === 901) {
                    const lastItem = _.last(lists);
                    return Number(lastItem.outlineCatalog || -1);
                }
                return lists.length;
            },
            ...
            getLastOrderNum(treeData = {}, outlineType = 0, isAppendix = false) {
                var orderNum = 0;
                var sameNodes = _.filter(treeData.children, {
                    parentId: treeData.outlineId,
                ...
                if (isAppendix) {
                    sameNodes = _.filter(treeData.children, (item) => {
                        return (
                            item.parentId === treeData.outlineId && [8, 9].includes(item.outlineType) &&
                            item.isVisible === 1
                        );
                    });
                }
            ...
            resetTreeAfterDrag(droppedData, targetData, type) {
                ...
                this.$emit("change", {
                    act: "updateOutline",
                    data: { tree: this.treeData, droppedData, targetData, type },
                });
            },
            /**
             *  @description 重置数据
            ...
            resetTreeData() {
                var treeData = this.treeData[0];
                treeData.children = _.orderBy(treeData.children, [
                    "outlineType",
                    "orderNum",
                    "outlineCatalog",
                ]);
            ...
            resetOutline(
                startOutlineCatalog = 0,
                orders = ["outlineType", "outlineCatalog"]
            ) {
                        ...
                        if (node.outlineCatalog) {
                            if (!parentNode && ![3, 4, 5, 6, 8, 9].includes(node.outlineType)) {
                                ...
                                /* if ([8, 9].includes(node.outlineType)) {
                                                      node.outlineCatalog = $global.numberToLetters(outlineCatalog-1);
                                                  } */
                            } else {
                                // 根据新位置生成新的outlineCatalog，ancestors，letter
                                if (parentNode) {
                                    if (parentNode.letter) {
                                        // [8, 9].includes(parentNode.outlineType) &&
                                        node.letter = parentNode.letter; // + '.' + (idx + 1);
                                    }
                                    node.outlineCatalog =
                                        parentNode.outlineCatalog + "." + (idx + 1);
                            ...
                            if (node.orderNum > orderNumStart + 99) {
                                orderNum =
                                    String(node.outlineCatalog).length === 1 ?
                                    parseInt(orderNum / 100 + 1) * 100 :
                                    orderNum + 1;
                            ...
                            if (node.children && node.children.length) {
                                node.children = _.orderBy(
                                    node.children,
                                    orders,
                                    new Array(orders.length).fill("asc")
                                );
                ...
                this.treeData[0].children.forEach((item) => {
                    if ([3, 4, 5, 6, 8, 9].includes(item.outlineType)) {
                        if ([8, 9].includes(item.outlineType)) {
                            asourceIndex++;
                            let letter = $global.numberToLetters(asourceIndex - 1);
                            item.letter = letter;
                            item.outlineCatalog = letter; //asourceIndex;
                        } else {
                            sourceIndex++;
            ...
            setCatalogNum(outlineCatalog = 0, orders = ["outlineType", "outlineCatalog"]) {
                // let outlineCatalog = outlineCatalog;
                if (outlineCatalog === 0 && this.stdKind === 901) { // 航司的从0开始计数
                    outlineCatalog = -1;
                }
                ...
                this.treeData[0].children.forEach((item) => {
                    if ([3, 4, 5, 6, 8, 9].includes(item.outlineType)) {
                        if ([8, 9].includes(item.outlineType)) {
                            ...
                            item.letter = $global.numberToLetters(asourceIndex - 1);
                        } else {
                            sourceIndex++;
                            item.outlineCatalog = sourceIndex;
                        }
            ...
            editDocbyUnlock(data = {}, node = {}) {
                ...
                if (this.author.lockedAll) {
                    ...
                    for (let i = 0; i < parentNodes.length; i++) {
                        if (
                            parentNodes[i]["locked"] &&
                            parentNodes[i].outlineId !== data.outlineId
                        ) {
                            msg =
                                "父章节条目【" +
                                parentNodes[i].outlineTitle +
                                "】已被锁定编辑，请在待解锁后再操作！";
                            break;
                        }
                    }
                    // 同时校验子章节是否有被锁定
                    if (!msg) {
                        var childNodes = $global.getChildNodes(data.children || []);
                        for (let i = 0; i < childNodes.length; i++) {
                            if (
                                childNodes[i]["locked"] &&
                                childNodes[i].outlineId !== data.outlineId
                            ) {
                                msg =
                                    "子章节条目【" +
                                    childNodes[i].outlineTitle +
                                    "】已被锁定编辑，请在待解锁后再操作！";
                                break;
                            }
                        }
                    ...
                    this.$emit("change", {
                        act: "editDoc",
                        data,
                        unlock: this.lockedBySelf(data, node),
                    });
                } else {
            ...
            async saveOutlineData(isTest = false) {
                ...
                var res = await batchUpdateOutline({ outlineList: lists },
                    editorConfig.editorURL || ""
                );
                if (res.code === 200) {
            ...
            checkAssignOutlieAuthor(node = null, data = null) {
                ...
                const matchOutline = (outlineItem) => {
                    var parentNode = $global.getParentNode(
                        this.treeData[0]["children"],
                        outlineItem.outlineId, { idKey: "outlineId", parentKey: "parentId", children: "children" }
                    );
                    for (let i = 0; i < parentNode.length; i++) {
                        if (this.assignOutlineId.includes(parentNode[i]["outlineId"])) {
                            // 同时包含自身节点 && parentNode[i]['outlineId'] !== outlineItem.outlineId
                            return true;
                        }
                    }
                    return false;
            ...
            assignOutlineToMember(node = null, data = null) {
                ...
                if (!isLocked) {
                    ...
                    // 校验子章节是否被锁定编辑的
                    if (!isLocked) {
                        let childNode = $global.findAllChildrenNode(data.children, data, {
                            key: "outlineId",
                            parentKey: "parentId",
                            children: "children",
                        });
            ...
            assignOutline(node = null, data = null) {
                ...
                const checkIsExits = (outlineData) => {
                    ...
                    if (!outlineData.owner) {
                        if (outlineData.children && outlineData.children.length) {
                            let allChildNodes = $global.findAllChildrenNode(
                                outlineData.children,
                                outlineData, { key: "outlineId", parentKey: "parentId", children: "children" }
                            );
                            for (let i = 0; i < allChildNodes.length; i++) {
                                let item = allChildNodes[i];
                                if (item.owner) {
                                    let outlineCatalog = item.letter ?
                                        "附录" + item.letter :
                                        item.outlineCatalog;
                        ...
                        var parentNodes = $global.getParentNode(
                            this.treeData[0].children,
                            outlineData.outlineId, { idKey: "outlineId", parentKey: "parentId", children: "children" }
                        );
                        if (parentNodes && parentNodes.length) {
                            for (let i = 0; i < parentNodes.length; i++) {
                                ...
                                if (
                                    item.owner &&
                                    _.isEmpty(
                                        this.assignOutlineId ||
                                        !this.assignOutlineId.includes(item.outlineId)
                                    )
                                ) {
                                    let outlineCatalog = item.letter ?
                                        "附录" + item.letter :
                ...
                if (checkIsExitsItems.length) {
                    this.$message({
                        ...
                        message: "所选章节条目有冲突（已被分配了章节条目），请重新定义协作分配！<br/>-----------------------------------------------------------------------------<br/>" +
                            checkIsExitsItems.join("<br/>"),
                    });
                    return false;
                }
                this.openDialogComp({
            ...
            submitDialogValue() {
                ...
                if (this.dialogData.cmpName === "assignOutline") {
                    // 分配大纲
                    var outlineData = $global.getChildData(
                        "outlineId",
                        this.dialogData.data.outlineId,
                        this.treeData[0].children
                    );
                    if (outlineData) {
                        outlineData.owner = this.memberList
                            .filter((o) => dataValue.includes(o.userId))
                            .map((item) => {
                ...
                } else if (this.dialogData.cmpName === "assignCollaboration") {
                    // 分配任务
                ...
                const setOwner = (arr) => {
                    arr.forEach((item) => {
                        let assignMembers = this.memberList.filter((user) => {
                            return (
                                user.assignOutline && user.assignOutline.includes(item.outlineId)
                            );
                        });
                        let owner = [];
            ...
            isLockParent(item = {}, node = {}) {
                let parentNodes = $global.getParentNode(
                    this.treeData[0].children,
                    data.outlineId, { idKey: "outlineId", parentKey: "parentId", children: "children" }
                );
                if (parentNodes && parentNodes.length) {
            ...
            getParentLockBySelf(item = {}, isSelf = false) {
                let parentNodes = $global.getParentNode(
                    this.treeData[0].children,
                    item.parentId, { idKey: "outlineId", parentKey: "parentId", children: "children" }
                );
                if (parentNodes && parentNodes.length) {
                    for (let i = 0; i < parentNodes.length; i++) {
                        let isLocked = this.lockedBySelf(parentNodes[i]);
                        if (
                            isLocked ||
                            (isSelf &&
                                !_.isEmpty(parentNodes[i]["owner"]) &&
                                this.checkIsAssignOutline(item))
                        ) {
                            return true;
                        }
                    }
            ...
            assignOwnerContent(data) {
                ...
                let userName = [];
                if (!_.isEmpty(data.owner)) {
                    let owner = _.isString(data.owner) ?
                        JSON.parse(data.owner) :
                        data.owner;
                    owner.forEach((item) => {
                        userName.push(item.userName);
        ...
        mounted() {
            ...
            // this.debouncedRefreshDoc = _.debounce(this.refreshDoc, 300);
        },
    };
</script>
