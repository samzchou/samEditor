        ...
        methods: {
            changeValues() {
                ...
                this.dataValues.forEach(val => {
                    ...
                    if (data) {
                        strArr.push(`<a data-id="${data.id}" data-name="draftUtil" data-code="${val}" title="${data.title}">${data.label}(${data.value})</a>`);
                    }
                });
            ...
            async getUtilData() {
                ...
                if (res.error_code === 200) {
                    res.data.forEach(item => {
                        if(item.secretariat) {
                            let obj = {
                                label: item.secretariat,
                                value: item.no
                            }
                            var index = _.findIndex(this.utilList, obj);
                            if(!~index) {
                                obj.id = item.id;
                                obj.title = item.scope + '-' + item.secretariat_secretry;
